      ...
      {commonSpecs.length === 0 && buyerISQs.length === 0 ? (
        <div className="bg-yellow-50 border border-yellow-200 p-6 rounded-lg text-yellow-800">
          ...
          <p className="text-sm mt-2">
            ...
            {stage1Data.audit?.length > 0 && (
              <span> Found {stage1Data.audit.length} audit specs.</span>
            )}
          </p>
        </div>
      ) : (
...
// ============================================
// IMPORTANT: Yeh updated function hai jismein Stage 1 ke audit specs use honge
// ============================================
function extractCommonAndBuyerSpecs(
  stage1: Stage1Output,
  isqs: { config: ISQ; keys: ISQ[]; buyers: ISQ[] }
): { commonSpecs: CommonSpecItem[]; buyerISQs: BuyerISQItem[] } {
  console.log('🔍 Stage3: Extracting common specs...');
  console.log('Stage1 data:', stage1);
  console.log('ISQs data:', isqs);

  // Collect all stage2 ISQs with priorities
  const stage2ISQs: (ISQ & { priority: number; normName: string })[] = [
    { ...isqs.config, priority: 3, normName: normalizeSpecName(isqs.config.name) },
    ...isqs.keys.map(k => ({ ...k, priority: 2, normName: normalizeSpecName(k.name) })),
    ...isqs.buyers.map(b => ({ ...b, priority: 1, normName: normalizeSpecName(b.name) }))
  ];

  console.log('Stage2 ISQs:', stage2ISQs);
  ...
  // IMPORTANT: Yahaan ab hum Stage 1 ke finalized specs ke saath audit specs bhi use kar rahe hain
  stage1.seller_specs.forEach((ss) => {
    ss.mcats.forEach((mcat) => {
      ...
      finalized_primary_specs.specs.forEach((spec) => {
        if (spec.spec_name && spec.options?.length > 0) {
          stage1AllSpecs.push({
  ...
  // Audit specs ko bhi add karo (agar available hain)
  if (stage1.audit && stage1.audit.length > 0) {
    console.log('Adding audit specs:', stage1.audit.length);
    stage1.audit.forEach((auditItem) => {
      // Audit specs ko bhi add karo (priority 1)
      if (auditItem.specification) {
        stage1AllSpecs.push({
          spec_name: auditItem.specification,
          options: [], // Audit mein options nahi hote
          input_type: 'text',
          tier: 'Primary', // Default
          priority: 1,
          normName: normalizeSpecName(auditItem.specification)
        });
      }
    });
  }

  console.log('Total Stage1 specs:', stage1AllSpecs.length);
  stage1AllSpecs.forEach(s => console.log(`  - ${s.spec_name} (${s.tier})`));

  // Find common specs - EXACT NORMALIZED NAME MATCHING
  ...
  stage1AllSpecs.forEach((stage1Spec, i) => {
    // Find matching Stage2 ISQ using normalized names
    ...
    stage2ISQs.forEach((stage2ISQ, j) => {
      ...
      // Check for exact normalized name match OR semantic similarity
      const isMatch = stage1Spec.normName === stage2ISQ.normName || 
                     isSemanticallySimilar(stage1Spec.spec_name, stage2ISQ.name);
    ...
    if (bestMatchIndex !== -1) {
      ...
      console.log(`✅ Found common: ${stage1Spec.spec_name} ↔ ${stage2ISQs[bestMatchIndex].name}`);
    }
  });
  ...
  console.log('Final common specs:', uniqueCommonSpecs.length);
  ...
  console.log('Final buyer ISQs:', optimizedBuyerISQs.length);
